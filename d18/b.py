...
def solve(input_file: pathlib.Path) -> str:
    ...
    bytes_fallen = set()
    last_byte_position: Point = (0, 0)

    while True:
        last_byte_position = byte_positions.pop(0)
        bytes_fallen.add(last_byte_position)
        ...
        if path is None:
            result = f"{last_byte_position[0]},{last_byte_position[1]}"
            break
